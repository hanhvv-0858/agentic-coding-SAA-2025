...
export function KudoPostCard({
  ...
  return (
    ...
    >
      {/* Sender → send-icon → primary recipient row. Icon sits at the
          vertical center of the 64 px avatar row (§17 design — the
          "send" glyph aligns with the avatars, not the full participant
          column height). */}
      ...
      {/* Timestamp (§17d, left) + Edit pencil (§17, right — sender-only
          edit affordance). Sits on the card surface above the title. */}
      <div className="flex w-full items-center justify-between gap-4">
        ...
        {isSender ? (
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded border border-[var(--color-muted-grey)]/40 text-[var(--color-brand-900)] transition hover:bg-[var(--color-brand-900)]/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-accent-cream)]"
          >
      ...
      {/* Kudo title (§17 — "IDOL GIỚI TRẺ") centered, navy. */}
      {kudo.title ? (
        <p
          className="w-full text-center font-[family-name:var(--font-montserrat)] text-lg font-bold leading-6 tracking-wide text-[var(--color-brand-900)]"
      ...
      >
        <p
          className="line-clamp-4 w-full cursor-pointer whitespace-pre-line text-center font-[family-name:var(--font-montserrat)] text-[20px] font-bold leading-7 text-[var(--color-brand-900)]"
